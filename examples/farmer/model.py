...
model.Yield = pyo.Param(
    CROPS, initialize={'WHEAT': 2.5, 'CORN': 3, 'BEETS': 20})
...
def profit_rule(model):
    return sum(model.SubQuotaSellingPrice[crop] * model.QuantitySubQuotaSold[crop] for crop in CROPS) + \
           sum(model.SuperQuotaSellingPrice[crop] * model.QuantitySuperQuotaSold[crop] for crop in CROPS) - \
           sum(model.PurchasePrice[crop] * model.QuantityPurchased[crop] for crop in CROPS) - \
           sum(model.PlantingCostPerAcre[crop] * model.DevotedAcreage[crop] for crop in CROPS)
...
def crop_selling_rule(model, crop):
    return model.QuantitySubQuotaSold[crop] + model.QuantitySuperQuotaSold[crop] + \
           model.QuantityRemainder[crop] == model.Yield[crop] * model.DevotedAcreage[crop]
model.crop_selling_constraint = pyo.Constraint(CROPS, rule=crop_selling_rule)

def cattle_feed_rule(model, crop):
    return model.QuantityRemainder[crop] + model.QuantityPurchased[crop] >= model.CattleFeedRequirement[crop]
