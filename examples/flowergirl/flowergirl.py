...
def stage(
    ...
    block.c1 = pyo.Constraint(
        expr=prev_inventory - demand == block.inventory_plus - block.inventory_minus
    )
    block.c2 = pyo.Constraint(
        expr=block.next_inventory == block.purchase + loss * block.inventory_plus
    )
