...
def first_stage(model: pyo.ConcreteModel, nJ: int, seed: int = 1):
    ...
    model.obj = pyo.Objective(
        expr=sum(model.c[j] * model.x[j] for j in model.sJ), sense=pyo.minimize
    )


def second_stage(
    block: pyo.ScalarBlock, x: pyo.Var, nI: int, nJ: int, seed1: int = 2, seed2: int = 3
):
    random.seed(seed1)
    ...
    def rule_demand(block, j):
        return (
            sum(block.d[i, j] * block.y[i, j] for i in block.sI) - block.y0[j]
            <= u * x[j]
        )
    ...
    block.objexpr = sum(block.q0[j] * block.y0[j] for j in block.sJ) - sum(
        block.q[i, j] * block.y[i, j] for i in block.sI for j in block.sJ
    )
