...
class CutAggregator:
    def __init__(self, groups: List[List[NodeIdx]], multipliers: Dict[NodeIdx, float]) -> None:
        self.groups = groups
        self.group_multipliers = [[multipliers[idx] for idx in group] for group in groups]
    ...
    def get_aggregate_cuts(self, cuts: Dict[int, Cut]) -> List[CutList]:
        ...
        for i, group in enumerate(self.groups):
            ...
            aggregate_cut = self._get_aggregate_cut(self.group_multipliers[i], group_cut)
